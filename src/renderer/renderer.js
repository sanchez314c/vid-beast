...
function initializeEventListeners() {
  ...
  if (aboutGithubLink) {
    aboutGithubLink.addEventListener('click', (e) => {
      e.preventDefault();
      const url = 'https://github.com/sanchez314c/vid-beast';
      try {
  ...
  const enableAdvancedRepairEl = document.getElementById('enableAdvancedRepair');
  if (enableAdvancedRepairEl) {
    enableAdvancedRepairEl.addEventListener('change', function() {
      const advancedRepairOptionsEl = document.getElementById('advancedRepairOptions');
      if (advancedRepairOptionsEl) advancedRepairOptionsEl.style.display = this.checked ? 'block' : 'none';
    });
  } else {
    console.warn('enableAdvancedRepair element not found');
  }

  // Frame extraction on failure options
  const extractFramesOnFailureEl = document.getElementById('extractFramesOnFailure');
  if (extractFramesOnFailureEl) {
    extractFramesOnFailureEl.addEventListener('change', function() {
      const frameExtractionOptionsEl = document.getElementById('frameExtractionOptions');
      if (frameExtractionOptionsEl) frameExtractionOptionsEl.style.display = this.checked ? 'block' : 'none';
    });
  } else {
    console.warn('extractFramesOnFailure element not found');
  }
}
...
ipcRenderer.on('batch-progress', (event, data) => {
  ...
  // NOTE: stats are already updated in the 'completed' branch above.
  // This block was duplicating every count — removed to fix double-counting bug.
  ...
  try {
    ...
    if (result.type === 'folder') {
      ...
      try {
        ...
        updateStatusBarItems(files.length);
      ...
      updateStatusBarItems(result.files.length);
    }
  ...
  try {
    ...
    if (!legendContainer) {
      ...
      const legendItems = [
        ...
        { text: 'Corrupted', color: '#ef4444' },
        { text: 'Repaired', color: '#14b8a6' }
      ];
...
function drawProgressCircle() {
  try {
    ...
    ctx.strokeStyle = '#1e1e24'; // Theme border-subtle
    ...
    if (total > 0) {
      ...
      if (progressChart.data.healthy > 0) {
        ...
        ctx.strokeStyle = '#10b981'; // Theme success green
        ctx.lineWidth = lineWidth;
        ctx.stroke();
      ...
      if (progressChart.data.corrupted > 0) {
        ...
        ctx.strokeStyle = '#ef4444'; // Theme error red
        ctx.lineWidth = lineWidth;
        ctx.stroke();
      ...
      if (progressChart.data.repaired > 0) {
        ...
        ctx.strokeStyle = '#14b8a6'; // Theme accent teal
        ctx.lineWidth = lineWidth;
        ctx.stroke();
      }
    }
...
function drawMovingSegments(ctx, centerX, centerY, radius, lineWidth) {
  ...
  ctx.strokeStyle = '#2a2a30'; // Theme border-light
  ctx.lineWidth = lineWidth;
...
// Convenience wrapper — called from settings gear onclick in title bar
...
async function startRepairs() {
  ...
  for (let i = 0; i < repairQueue.length; i++) {
    ...
    if (!row) {
      console.warn(`Row not found in DOM for file: ${item.file}, skipping`);
      continue;
    }
...
async function repairSingleFile(filePath) {
  try {
    ...
    const outputDir = outputFolderEl ? outputFolderEl.value : '';
    if (!outputDir || outputDir.trim() === '') {
      updateStatus('Please select an output folder first');
      return;
    }
...
function generateHTMLReport(results) {
  ...
  let html = `
    ...
    <head>
        ...
        <style>
            body { font-family: 'Inter', Arial, sans-serif; margin: 20px; background: #0a0b0e; color: #e8e8ec; }
            .header { background: linear-gradient(145deg, #141518, #18191c); color: #e8e8ec; padding: 20px; border-radius: 14px; border: 1px solid #1e1e24; }
            ...
            th, td { border: 1px solid #1e1e24; padding: 12px; text-align: left; }
            th { background: #0a0b0e; color: #14b8a6; }
            .healthy { color: #10b981; font-weight: bold; }
            .corrupted { color: #ef4444; font-weight: bold; }
            .repairable { color: #f59e0b; font-weight: bold; }
  ...
  results.forEach(result => {
    ...
    const issues = result.issues ? result.issues.map(escapeHTML).join(', ') : 'None';
    const recommendations = result.recommendations ? result.recommendations.map(escapeHTML).join('; ') : 'None';
...
function generateCSVReport(results) {
  // Escape CSV field: wrap in quotes, escape internal quotes by doubling them
  ...
  results.forEach(result => {
    ...
    csv += `${csvField(fileName)},${csvField(status)},${csvField(result.corruptionLevel || 'unknown')},${csvField(repairable)},${csvField(issues)},${csvField(recommendations)}\n`;
  });
...
function updateResultsTab(results) {
  ...
  if (!resultsDetails) {
    console.warn('resultsDetails element not found');
    return;
  }
    ...
    const detailsHTML = results.map(result => {
      ...
      return `
                <div class="result-item ${statusClass}">
                    <h4>${fileName}</h4>
                    <p><strong>Status:</strong> ${result.corruptionLevel === 'none' ? 'Healthy' : 'Corrupted (' + corruptionLevel + ')'}</p>
                    <p><strong>Repairable:</strong> ${result.repairFeasible ? 'Yes' : 'No'}</p>
                    ${result.issues && result.issues.length > 0 ? '<p><strong>Issues:</strong> ' + result.issues.map(escapeHTML).join(', ') + '</p>' : ''}
                    ${result.recommendations && result.recommendations.length > 0 ? '<p><strong>Recommendations:</strong><br>' + result.recommendations.map(r => '• ' + escapeHTML(r)).join('<br>') + '</p>' : ''}
                </div>
            `;
    }).join('');
...
function updateStatus(message) {
  try {
    ...
    // Update canonical status bar footer
    const statusBarText = document.getElementById('statusBarText');
    if (statusBarText) statusBarText.textContent = 'Status: ' + message;
...
function updateStatusBarItems(count) {
  const el = document.getElementById('statusBarItems');
  if (el) el.textContent = count + (count === 1 ? ' file' : ' files');
}
...
async function checkFFmpegStatus() {
  try {
    ...
    if (ffmpegInfo.isInstalled) {
      ...
      statusElement.innerHTML = `
                <span style="color: var(--warning);">FFmpeg Not Found</span><br>
                <span style="font-size: 12px; color: var(--text-muted);">Will be automatically downloaded on first use</span>
            `;
    }
  } catch (error) {
...
async function checkHardwareAcceleration() {
  try {
    ...
    if (settingsGroup) {
      ...
      hwInfo.style.cssText = 'margin-top: 8px; padding: 10px; background: var(--bg-void); border: 1px solid var(--border-subtle); border-radius: 6px; font-size: 12px;';
      ...
      if (hwAccelInfo.available) {
        ...
        hwInfo.innerHTML = `
                    <span style="color: var(--text-dim);">Hardware Acceleration Not Available</span><br>
                    <span style="color: var(--text-dim);">CPU encoding will be used</span>
                `;
        if (gpuCheckbox) {
...
function updateHeaderHardwareIndicator(hwAccelInfo) {
  ...
  if (hwAccelInfo.available) {
    gpuIndicator.innerHTML = '<span style="color: var(--success);">GPU Accel</span>';
    gpuIndicator.title = `Hardware acceleration available: ${hwAccelInfo.gpuInfo || 'GPU detected'}`;
  } else {
    gpuIndicator.innerHTML = '<span style="color: var(--text-dim);">CPU Only</span>';
...
function updateSidebarStats(stats, totalFiles) {
  ...
  if (corruptedCountSpan) {
    ...
    if (totalCorrupted > 0) {
      corruptedCountSpan.style.color = 'var(--warning)';
    }
  }
  ...
  if (repairableCountSpan) {
    ...
    if (stats.repairable > 0) {
      repairableCountSpan.style.color = 'var(--success)';
    }
  }
  ...
  if (successRateSpan) {
    ...
    if (successRate >= 80) {
      successRateSpan.style.color = 'var(--success)';
    } else if (successRate >= 50) {
      successRateSpan.style.color = 'var(--warning)';
    } else if (successRate > 0) {
      successRateSpan.style.color = 'var(--error)';
    }
  }
...
// Add CSS overrides for dynamic elements (Dark Neo Glass theme)
const style = document.createElement('style');
style.textContent = `
/* Dynamic element overrides — all colors use theme variables */
#progressChart {
    filter: drop-shadow(0 4px 8px rgba(0, 0, 0, 0.3));
}

.analyzing #progressChart {
    filter: drop-shadow(0 4px 8px rgba(0, 0, 0, 0.3)) drop-shadow(0 0 10px rgba(20, 184, 166, 0.3));
}
...
/* Hardware accel info dynamic panel */
.hw-accel-info {
    margin-top: 8px;
    padding: 10px;
...
// ── Window Controls IPC Wiring (Dark Neo Glass) ──
