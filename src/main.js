...
const VERBOSE_PROGRESS = false; // Set to true only for deep ffmpeg debugging
...
function validatePath(inputPath) {
  ...
  // Prevent path traversal attacks — check for '..' as a path component
...
async function checkBundledFFmpeg() {
  try {
    ...
    // Test if they work - use spawn (not exec) to avoid shell injection
    const ffmpegResult = await new Promise((resolve) => {
      const proc = spawn(ffmpegPath, ['-version']);
      let out = '';
      proc.stdout.on('data', (d) => { out += d.toString(); });
      proc.on('error', () => resolve(false));
      proc.on('close', (code) => resolve(code === 0 || out.includes('ffmpeg version')));
    });
    ...
    const ffprobeResult = await new Promise((resolve) => {
      ...
      proc.stdout.on('data', (d) => { out += d.toString(); });
      proc.on('error', () => resolve(false));
      proc.on('close', (code) => resolve(code === 0 || out.includes('ffprobe version')));
    });
...
async function findSystemFFmpeg() {
  ...
  for (let i = 0; i < commonPaths.length; i++) {
    ...
    try {
      const ffmpegResult = await new Promise((resolve) => {
        const proc = spawn(ffmpegTestPath, ['-version']);
        let out = '';
        proc.stdout.on('data', (d) => { out += d.toString(); });
        proc.on('error', () => resolve(false));
        proc.on('close', (code) => resolve(code === 0 || out.includes('ffmpeg version')));
      ...
      const ffprobeResult = await new Promise((resolve) => {
        const proc = spawn(ffprobeTestPath, ['-version']);
        let out = '';
        proc.stdout.on('data', (d) => { out += d.toString(); });
        proc.on('error', () => resolve(false));
        proc.on('close', (code) => resolve(code === 0 || out.includes('ffprobe version')));
...
// ── Window Control IPC Handlers (Dark Neo Glass) ──
...
// Open external URLs safely — protocol validation prevents arbitrary URL schemes
ipcMain.handle('open-external', async (event, url) => {
  try {
    ...
    console.error('Invalid URL passed to open-external:', url, error.message);
  }
});
...
ipcMain.handle('get-ffmpeg-info', async () => {
  try {
    ...
    if (isInstalled) {
      // Get FFmpeg version - use spawnPromise to avoid shell injection
      try {
        const { stdout } = await spawnPromise(ffmpegPath, ['-version']);
...
ipcMain.handle('check-hw-acceleration', async () => {
  try {
    ...
    // Check for hardware encoders - use spawnPromise to avoid shell injection
    const { stdout } = await spawnPromise(ffmpegPath, ['-hide_banner', '-encoders']);
...
// Helper function for promisified exec (retained for legacy callers)
...
// Safe spawn-based alternative to execPromise — avoids shell injection
